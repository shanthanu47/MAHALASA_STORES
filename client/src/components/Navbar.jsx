...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-stone-200 bg-tulunad-cream/95 backdrop-blur-md sticky top-0 z-50 transition-all">

      <NavLink to='/' onClick={() => setOpen(false)} className="flex items-center gap-2 md:gap-3">
        <img className="h-14" src={assets.logo} alt="logo" />
        <span className="text-xl md:text-2xl font-serif font-bold text-tulunad-primary tracking-wide">
      ...
      <div className="hidden sm:flex items-center gap-8 font-medium text-stone-700">
        <NavLink to='/' className={({ isActive }) => isActive ? "text-tulunad-primary" : "hover:text-tulunad-primary transition"}>Home</NavLink>
        <NavLink to='/products' className={({ isActive }) => isActive ? "text-tulunad-primary" : "hover:text-tulunad-primary transition"}>All Products</NavLink>
        <NavLink to='/contact' className={({ isActive }) => isActive ? "text-tulunad-primary" : "hover:text-tulunad-primary transition"}>Contact</NavLink>

        {/* Desktop Search - Large screens */}
        <div className="hidden lg:block relative">
          <form onSubmit={handleSearchSubmit} className="flex items-center text-sm gap-2 border border-stone-300 px-3 rounded-full bg-white/50 focus-within:bg-white transition-colors focus-within:border-tulunad-primary focus-within:ring-1 focus-within:ring-tulunad-primary/20">
            <input
        ...
        {!user ? (<button onClick={() => setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-tulunad-primary hover:bg-orange-800 transition text-white rounded-full shadow-md hover:shadow-lg">
          Login
        </button>)
          :
          (
      ...
      {open && (
        <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-3 px-5 text-sm md:hidden z-50`}>
          ...
          {!user ? (
            ...
            }} className="cursor-pointer px-6 py-2 mt-2 bg-tulunad-primary hover:bg-orange-800 transition text-white rounded-full text-sm">
              Login
            </button>
          ) : (
            <>
              <button onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-tulunad-primary hover:bg-orange-800 transition text-white rounded-full text-sm">
                Logout
              </button>
              <button onClick={() => {
                setOpen(false);
